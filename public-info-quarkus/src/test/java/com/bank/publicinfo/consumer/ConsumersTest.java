...
class ConsumersTest {
    ...
    @InjectMock CertificateService  certService;
    @InjectMock LicenseService      licenseService;
    ...
    @Inject CertificateConsumer  certConsumer;
    @Inject LicenseConsumer      licenseConsumer;
    @Inject ErrorLogsConsumer    errorConsumer;
    ...
    @Nested @DisplayName("BankDetailsConsumer")
    class BankDetails {

        @Test @DisplayName("onCreate: валидный JSON → create() + метрика++")
        void onCreate_valid() {
            bankConsumer.onCreate("{\"id\":1,\"bik\":123456789}");
            verify(bankService).create(any()); verify(ok).increment();
        }

        @Test @DisplayName("onUpdate: валидный JSON → update() + метрика++")
        void onUpdate_valid() {
            bankConsumer.onUpdate("{\"id\":1,\"bik\":123456789}");
            verify(bankService).update(any()); verify(ok).increment();
        }

        @Test @DisplayName("onDelete: валидный id → deleteById() + метрика++")
        void onDelete_valid() {
            bankConsumer.onDelete("{\"id\":5}");
            verify(bankService).deleteById(5L); verify(ok).increment();
        }

        @Test @DisplayName("onDelete: null id → сервис НЕ вызывается")
        void onDelete_nullId_skips() {
            bankConsumer.onDelete("{\"bik\":999}");
            verify(bankService, never()).deleteById(any()); verify(err, never()).increment();
        }

        @Test @DisplayName("onCreate: невалидный JSON → kafkaErrors++, без exception")
        ...
        @Test @DisplayName("onUpdate: невалидный JSON → kafkaErrors++")
    ...
    @Nested @DisplayName("BranchConsumer")
    class Branch {

        @Test @DisplayName("onCreate: валидный JSON → create() + метрика++")
        void onCreate_valid() {
            branchConsumer.onCreate("{\"id\":1,\"address\":\"ул. Ленина\",\"city\":\"Москва\"}");
            verify(branchService).create(any()); verify(ok).increment();
        }

        @Test @DisplayName("onUpdate: null id → сервис НЕ вызывается")
        void onUpdate_nullId_skips() {
            branchConsumer.onUpdate("{\"address\":\"ул. Ленина\"}");
            verify(branchService, never()).update(any(), any());
        }

        @Test @DisplayName("onUpdate: с id → update() + метрика++")
        void onUpdate_withId() {
            branchConsumer.onUpdate("{\"id\":3,\"address\":\"ул. Мира\",\"city\":\"СПб\"}");
            verify(branchService).update(eq(3L), any()); verify(ok).increment();
        }

        @Test @DisplayName("onDelete: с id → deleteById() + метрика++")
        void onDelete_withId() {
            branchConsumer.onDelete("{\"id\":7}");
            verify(branchService).deleteById(7L); verify(ok).increment();
        }

        @Test @DisplayName("onCreate: невалидный JSON → kafkaErrors++")
        void onCreate_invalidJson() {
            branchConsumer.onCreate("BAD_JSON");
            verify(err).increment(); verify(branchService, never()).create(any());
        }
    }

    // ── ATMConsumer ───────────────────────────────────────────────────────────

    @Nested @DisplayName("ATMConsumer")
    class ATM {

        @Test @DisplayName("onCreate: валидный JSON → create() + метрика++")
        void onCreate_valid() {
            atmConsumer.onCreate("{\"id\":1,\"branchId\":2,\"allHours\":true}");
            verify(atmService).create(any()); verify(ok).increment();
        }

        @Test @DisplayName("onUpdate: с id → update() + метрика++")
        void onUpdate_withId() {
            atmConsumer.onUpdate("{\"id\":4,\"branchId\":1,\"allHours\":false}");
            verify(atmService).update(any()); verify(ok).increment();
        }
        ...
        void onDelete_withId() {
            ...
            verify(atmService).deleteById(9L); verify(ok).increment();
        }
    ...
    @Nested @DisplayName("LicenseConsumer")
    class License {
    ...
    @Nested @DisplayName("ErrorLogsConsumer")
    class ErrorLogs {

        @Test @DisplayName("handleErrorLog → не бросает исключений")
        ...
        @Test @DisplayName("handleErrorLog: null message → не бросает")
